...
for (let i = 1; i < blocks.length; i += 2) {
  ...
  // Find the end of this if block by matching return (...); }
  const returnEndIndex = rest.indexOf(';\n                        }');
  let altEndIndex = rest.indexOf(';\n                                  }'); // another indentation level
  let altEndIndex2 = rest.indexOf(';\n                      }'); // another indentation level

  let endIndex = -1;
  if (returnEndIndex !== -1 && (altEndIndex === -1 || returnEndIndex < altEndIndex) && (altEndIndex2 === -1 || returnEndIndex < altEndIndex2)) endIndex = returnEndIndex + 26;
  else if (altEndIndex !== -1 && (altEndIndex2 === -1 || altEndIndex < altEndIndex2)) endIndex = altEndIndex + 36;
  else endIndex = altEndIndex2 + 24;

  if (endIndex === -1 || endIndex < 20) {
    // try fallback regex
    const m = rest.match(/;\n\s*\}/);
    if (m) {
